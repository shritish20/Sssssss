...
async def live_dashboard(access_token: str = Query(...)):
    """Fetches live portfolio positions, option greeks (for Nifty 50), and P&L data."""
    config = await get_config(access_token)
    async with httpx.AsyncClient() as client:
        ...
        pnl_resp = await client.get(f"{config['base_url']}/trade/profit-loss/data", headers=config['headers'])
        pnl_resp.raise_for_status()
        pnl = pnl_resp.json()["data"]
